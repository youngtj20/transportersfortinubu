    ...
    (level: HeadingTagType) => {
      editor.update(() => {
        ...
        if ($isRangeSelection(selection)) {
          ...
          nodes.forEach((node) => {
            let targetNode = node;
            // Get the closest non-root parent
            while (targetNode && targetNode.getParent()?.getParent()) {
              targetNode = targetNode.getParent() as any;
            }
            
            if (targetNode && targetNode.getParent()) {
    ...
    (align: string) => {
      editor.update(() => {
        ...
        if ($isRangeSelection(selection)) {
          ...
          nodes.forEach((node) => {
            let targetNode = node;
            // Find the paragraph or block node
            while (targetNode && targetNode.getParent()?.getParent()) {
              targetNode = targetNode.getParent() as any;
            }
            if (targetNode && targetNode.getParent()) {
              targetNode.setStyle(`text-align: ${align}`);
            }
          });
        }
      });
          ...
          if (text.includes('<') && text.includes('>')) {
            ...
            children.forEach((child: any) => {
              ...
              if (childText.includes('<') && childText.includes('>')) {
                ...
                content += `<span style="color: ${format.color}">${childText}</span>`;
              } else if (format?.backgroundColor) {
                content += `<span style="background-color: ${format.backgroundColor}">${childText}</span>`;
              } else {
                content += childText;
              }
            });
